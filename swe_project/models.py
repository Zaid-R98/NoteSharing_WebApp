...
class User(db.Model):
    ...
    st_fa=db.Column(db.Boolean,nullable=False)

class Uni_admin(User,db.Model):
    uni_ad_id=db.Column(db.Integer,primary_key=True)
...
#Notes to check from:
"""  SWE Database Diagram

System admin- admin,admin

User
 id 		username	password 	email 	university_id	student/fa    

Uni admin(user)
id 	first name		last name		Gender

Student(user)
id	first name	last name		gender  	academic_level	age

Faculty(user)
id	first name		lastname		gender	age

Student-Course(user, course)
studentid	(fk)	courseid(fk)

University
id	name	city		country		uniadmin_id

College(university)
uni_id	 id_college	name	

Department(university)
uni_id	dept_id	collegeid		name

Course
deptid	courseid		code	name
...
Notes
note_id	course_id	  studentid	note(blob)	approved/disapp


Rating
ratingid	noteid	studentid	   rating(/10)"""
